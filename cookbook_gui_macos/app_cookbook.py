...
# log_path = '.' + '\\_logs_files\\' #works on windows
...
#main_databes_directory_path = '.' + '\\databes_files\\' # works on windows
#main_images_directory_path = '.' + '\\recipe_images\\' # works on windows
...
class TreeCategory(TreeView):
        ...
        if touch.button == 'right':
            ...
            # if bubble intancje doesn't exist, show bubble (code taken from Buuble example)
                ...
                if node and node.node_property == 'category':
                    self.bubble_instance = bubble_instance = BubbleMenu(item='category')
                elif node and node.node_property == 'recipe':
                    self.bubble_instance = bubble_instance = BubbleMenu(item='recipe')
                else:
                    bubble_instance = None

                bubble_instance.pos = node.pos  # set buuble position the same as selected node position
                self.add_widget(bubble_instance)
            else:
    ...
    #     if touch.button == 'right':
    #         if node and node.node_property == 'category':
    #             self.show_bubble_when_right_mouse_click(node, 'category')
    #             a = BubbleShowcase(node=node, category='category')
    #             self.add_widget(a)
    #         if node and node.node_property == 'recipe':
    #             self.show_bubble_when_right_mouse_click(node, 'recipe')
    #         return True
    #     else:  # if LEFT click
    #         # # if left click (and others) and some bubble is active, close bubble
    #         # if hasattr(self, 'buuble_instance'):
    ...
    # def show_bubble_when_right_mouse_click(self, selected_node, node_type):
    #         if not hasattr(self, 'bubble_instance'):
    #             # if bubble intancje doesn't exist, show bubble (code taken from Buuble example)
    #             if node_type == 'category':
    #                 self.bubble_instance = bubble_instance = RightMouseClickOnCategory(topwidget = self)
    #             elif node_type == 'recipe':
    #                 self.bubble_instance = bubble_instance = RightMouseClickOnRecipe(topwidget = self)
    #             else:
    #                 bubble_instance = None
    #
    #             bubble_instance.pos = selected_node.pos # set buuble position the same as selected node position
    #             self.add_widget(bubble_instance)
    #             print('a', bubble_instance.content)
    #
    #         else:
    #             # if bubble intancje exists, close it (remove widget and kill instance)
    #             self.remove_widget(self.bubble_instance)
...
#     topwidget = ObjectProperty(None)  # Holds a reference to the TreeView this Bubble belongs to
...
#             # if bubble intancje doesn't exist, show bubble (code taken from Buuble example)
#
#             if self.node and self.node.node_property == 'category':
#                 self.bubble_instance = bubble_instance = RightMouseClick(topwidget=self, item='category')
#             elif self.node and self.node.node_property == 'recipe':
#                 self.bubble_instance = bubble_instance = RightMouseClick(topwidget=self, item='recipe')
#             else:
#                 bubble_instance = None
#
#             bubble_instance.pos = self.node.pos  # set buuble position the same as selected node position
#             self.add_widget(bubble_instance)
#         else:
#             # if bubble intancje exists, close it (remove widget and kill instance)
#             self.remove_widget(self.bubble_instance)
...
class BubbleMenu(Bubble):
    topwidget = ObjectProperty(None)
    item = StringProperty(None) # refers to the item we click on: on treeview: recipe or category, on recipe_view: ingredients or description
    ...
    def show_buttons(self):
        if self.item == 'category':
            ...
            btn_change_name = BubbleButton(text = "Zmień nazwę")
            btn_delete = Button(text = 'Usuń')

            btn_change_name.bind(on_press = lambda a: print('aa'))
            btn_delete.bind(on_press = lambda a: print('b'))

            self.add_widget(btn_change_name)
            self.add_widget(btn_delete)

        elif self.item == 'recipe':
            btn_edit = BubbleButton(text = "Edytuj")
            btn_change_name = BubbleButton(text = "Zmień nazwę")
            btn_delete = BubbleButton(text = 'Usuń')
            # btn_edit.bind(on_press = print('c'))
            # btn_change_name.bind(on_press = print('a'))
            # btn_delete.bind(on_press = print('b'))
            self.add_widget(btn_change_name)
            self.add_widget(btn_delete)
            self.add_widget(btn_edit)
        else: # self.item == recipe_view
            btn_copy = BubbleButton(text = 'Kopiuj')
            btn_copy.bind(on_press = lambda a: self.topwidget.copy_label_text('label_description'))
    ...
    def on_touch_down(self, touch):
        ...
        if touch.button == 'right':
            if not hasattr(self, 'bubble_instance'):
                ...
                # if self.selected_recipe.node_property == 'category':
                self.bubble_instance = bubble_instance = BubbleMenu(topwidget = self, item='recipe_view')
                # else:
                #     bubble_instance = None

                bubble_instance.pos = self.selected_recipe.pos  # set buuble position the same as selected node position
                self.add_widget(bubble_instance)
            else:
                # if bubble intancje exists, close it (remove widget and kill instance)
                self.remove_widget(self.bubble_instance)
        ...
        #     pass
        #     return super(TreeCategory, self).on_touch_down(touch)
        return super(RecipeView, self).on_touch_down(touch)
