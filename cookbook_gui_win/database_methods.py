...
class DatabaseManager():
    ...
    def adding_to_db(self, table_name, table_column, value):
        ...
        while True:
            try:
                if type(value) == str: # when value is string, there is only one column that we want to add
                    self.cursor.execute("INSERT INTO {}({}) VALUES ('{}')".format(table_name, table_column, value))
                else: # when value is tuple (multiple addind to db) conn.execute("INSERT INTO COMPANY (ID,NAME,AGE,ADDRESS,SALARY) VALUES (1, 'Paul', 32, 'California', 20000.00 )");
                    self.cursor.execute("INSERT INTO {}({}) VALUES {}".format(table_name, table_column, value))
                break
